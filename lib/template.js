...
const templateConfigName = 'config.js'
...
function readTemplateConfig(templateConfigPath) {
  ...
  return require(templateConfigPath).default
}
...
function getComponentConfigsWidthRelativePath(basePath, componentConfigs) {
  return componentConfigs.map(componentConfig => {
    ...
    return {
      ...
      configRelativePath: pathParts.slice(0, pathParts.length - 1).concat(templateConfigName.replace('.js', '')).join('/'),
    }
  })
}
...
function readHistoryPagesWithoutPreview() {
  const readPageConfigs = createReadFiles(filePath => {
    if (!isDirectory(filePath)) return

    const { name } = path.parse(filePath)
    const previewFilePath = path.resolve(config.pagesPath, name, previewName)
    const configFilePath = path.resolve(config.pagesPath, name, pageConfigName)
    if (isFileExist(previewFilePath) || !isFileExist(configFilePath)) return
...
function createMockData(data) {
  const template = readFileContent(path.resolve(__dirname, 'templates/mockData.ejs'))
  const code = ejs.render(template, { data })
  const distPath = path.resolve(config.editorPath, 'mock.js')
  writeFileContent(distPath, prettierCode(code, distPath))
}
