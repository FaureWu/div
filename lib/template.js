 ...
 * 通过模版生成页面
 */
function writePageTemplate(templateConfig, data, preview = false) {
  const { page, ...params } = data
  const previewConfigFilePath = path.resolve(config.pagesPath, page.config.name.value, '.preview')
  ...
  if (preview) {
    console.log(previewConfigFilePath)
...
function readPrePages() {
  if (!isFileExist(config.tmpPath)) return []
  ...
  return readFiles(config.tmpPath)
}
